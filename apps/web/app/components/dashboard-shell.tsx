...
export function DashboardShell({ role, active, title, eyebrow, actions, children }: { role: Role; active: string; title: string; eyebrow: string; actions?: ReactNode; children: ReactNode }) {
  return <main className={`dash-shell dash-${role}`}><aside className="dash-sidebar"><a className="dash-logo" href="/"><span>Wed</span>Connect</a><p className="workspace-label">{labels[role]}</p><nav aria-label={`${role} dashboard navigation`}>{nav[role].map(([icon,label,href])=><a className={label===active?'active':''} href={href} key={label}><i aria-hidden="true">{icon}</i><span>{label}</span></a>)}</nav><div className="sidebar-help"><span>Need a hand?</span><p>Our support team is here for you.</p><a href="/support/grievance">Get support →</a></div><a className="back-home" href="/">← Back to website</a></aside><section className="dash-main"><header className="dash-topbar"><div><p>{eyebrow}</p><h1>{title}</h1></div><div className="dash-actions">{actions}<AccountNav workspace/></div></header><div className="dash-content">{children}</div></section></main>;
}
